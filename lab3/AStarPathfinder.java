...
public class AStarPathfinder {
    ...
    /**
     * Attempts to compute a path that navigates between the start and end locations
     * of the specified map. If a path can be found, the waypoint of the
     * <em>final</em> step in the path is returned; that waypoint can be used to
     * walk backwards to the starting point. If no path can be found,
        ...
        for (int y = loc.yCoord - 1; y <= loc.yCoord + 1; y++) {
            for (int x = loc.xCoord - 1; x <= loc.xCoord + 1; x++) {
                ...
                float prevCost = currWP.getPreviousCost()
                        + estimateTravelCost(currWP.getLocation(), nextWP.getLocation());
    ...
    /**
     * Estimates the cost of traveling between the two specified locations. The
     * actual cost computed is just the straight-line distance between the two
     * locations.
     **/
